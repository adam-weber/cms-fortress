IT Oxygen Instructions
1. bundle install (NO Update!)
2. rake db:drop db:create db:migrate db:seed (@adam-weber has the seed data available...it's gitignored due to sensitive information)
  a. This will seed the DB with the IT Oxygen site and hostname only! (You may need to change the hostname in db/seed)
3. Start server and login with Google account to create your user in the database. You will be automagically created as a default user with no roles or priveleges.
4. Logout of yourself and login with administrator credentials defined in migration 01
5. Make your gmail user account a super user!
6. Remind @adam-weber to change the host name of the google API and to provide you with the keys
7. That should be all...

Authentication
===============
**Username:** admin@itoxygen.com
**Password:** 1234qwer
